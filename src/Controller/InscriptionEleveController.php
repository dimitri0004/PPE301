...
class InscriptionEleveController extends AbstractController
{
    ...
    public function ajouter(Request $request, UserPasswordHasherInterface $userPasswordHasher, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('inscription_eleve_ajouter');
        }
    ...
    public function modifier(Request $request, $id, UserPasswordHasherInterface $userPasswordHasher, EntityManagerInterface $entityManager): Response
        {

            $user = $entityManager->getRepository(UsersEleves::class)->find($id);
        ...
        public function supprimer($id,UsersElevesRepository $usersElevesRepository, EntityManagerInterface $entityManagerInterface): Response
        {
            $user = $entityManagerInterface->getRepository(UsersEleves::class)->find($id);
            $supprimer=$usersElevesRepository->remove($user, true);
            return $this->redirectToRoute('inscription_eleve_liste');
        } 
